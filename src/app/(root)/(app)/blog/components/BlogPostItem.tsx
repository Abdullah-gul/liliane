...
const BlogPostItem: React.FC<{ post: BlogComment }> = ({ post }) => {
  ...
  return (
    <div style={{ cursor: 'pointer' }}>
      ...
      <h4 className="line-clamp-2 text-sm opacity-60 dark:text-neutral-400" onClick={handleOpenModal}>
        {post.content}
      </h4>
      ...
      <Modal
        ...
        content={
          <>
            <h3 className="text-lg md:text-xl">{post.title}</h3>
            <p className="text-sm opacity-60 dark:text-neutral-400">{post.content}</p>
          </>
        }
